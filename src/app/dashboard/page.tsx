...
type DocStatus = 'Submitted' | 'Pending' | 'Rejected';
...
type ReqRecord = { status: DocStatus; fileName: string | null; filePath: string | null; rejectionNote: string | null };
...
function RequirementRow({ label, note, Icon, record, uploading, onUpload, onView, viewing }: {
  ...
  return (
    <div className={cn('p-3.5 bg-white rounded-xl border shadow-[0_1px_3px_rgba(0,0,0,0.06)]', isRejected ? 'border-red-200' : 'border-[#E5E9EC]')}>
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-xs mt-0.5 truncate" style={{ color: '#6B7A8D' }}>{isSubmitted || isRejected ? record!.fileName : note}</div>
        </div>
        ...
        {isRejected ? (
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            <span className="flex items-center gap-1 text-xs font-semibold px-2.5 py-1 rounded-full bg-red-100 text-red-700 shrink-0">
              <AlertCircle className="w-3 h-3" /> Rejected
            </span>
            <button type="button" onClick={() => inputRef.current?.click()} disabled={uploading}
              className="text-xs font-semibold px-2.5 py-1 rounded-full text-white shrink-0" style={{ backgroundColor: '#DC2626' }}>
      ...
      {isRejected && (
        <div className="mt-2.5 flex items-start gap-2 rounded-lg bg-red-50 border border-red-100 p-2.5 text-xs text-red-800">
          <AlertCircle className="w-3.5 h-3.5 shrink-0 mt-0.5" />
          <span>{record?.rejectionNote?.trim() ? record.rejectionNote : 'HR rejected this document. Please review and re-upload.'}</span>
        </div>
      )}
...
export default function ApplicantDashboard() {
  ...
  const fetchRequirements = React.useCallback(() => {
    ...
    fetch(`/api/requirements?application_id=${encodeURIComponent(application.id)}`)
      ...
      .then((records: { document: string; status: string; file_name: string | null; file_path: string | null; rejection_note: string | null }[]) => {
        ...
        for (const r of records) {
          ...
          //
          // 'Rejected' has to be checked FIRST and kept as its own status -
          // previously it fell into the "anything that isn't 'Not Submitted'"
          // bucket below and got mislabeled 'Submitted', so a rejected file
          // still showed the green "Submitted" check with no indication
          // anything was wrong, and re-uploading looked exactly the same
          // as before (still just "Submitted"), so nothing ever visibly
          // changed for the applicant.
          const status: DocStatus =
            r.status === 'Rejected' ? 'Rejected'
            : r.status && r.status !== 'Not Submitted' ? 'Submitted'
            : 'Pending';
          map[r.document] = {
            status,
            fileName: r.file_name,
            filePath: r.file_path,
            rejectionNote: status === 'Rejected' ? r.rejection_note : null,
          };
        }
  ...
  // 'Pending' is the placeholder value HR/the DB sets the moment `stage`
  // moves to "Job Offer" (before the applicant has actually clicked
  // Accept/Decline) - it must NOT count as "responded", or the gate below
  // treats the offer as already handled and the applicant gets skipped
  // straight to Requirements without ever seeing the Accept/Decline screen.
  const jobOfferResponded = application?.job_offer_response === 'Accepted' || application?.job_offer_response === 'Declined';
